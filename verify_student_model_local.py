#!/usr/bin/env python3
"""
verify_student_model_local.py -- Pure NumPy replica of the student GRU
seq2seq model, using the RAW (pre-quantization) float32 weights extracted
by extract_student_weights.py, to check whether the trained model itself
(bypassing all int8 quantization) reproduces expected_out_k.csv.
...
def main():
    ...
    required_keys = [
        "sencgru_kernel", "sencgru_recurrent_kernel", "sencgru_bias",
        "sdecgru_kernel", "sdecgru_recurrent_kernel", "sdecgru_bias",
    ...
    enc_kernel = data["sencgru_kernel"]
    enc_recurrent = data["sencgru_recurrent_kernel"]
    enc_bias = data["sencgru_bias"]
    ...
    dec_kernel = data["sdecgru_kernel"]
    dec_recurrent = data["sdecgru_recurrent_kernel"]
    dec_bias = data["sdecgru_bias"]
    ...
    dense_kernel = data["sdec_dense_kernel"]
    dense_bias = data["sdec_dense_bias"]
    preds = dec_hidden_seq @ dense_kernel + dense_bias

    print("\nNumPy replica predictions, t=0..5:")
    for t in range(6):
        print(f"  t={t}  {preds[t]}")
    ...
    if os.path.isfile(EXPECTED_OUT_CSV):
        ...
        print(f"\nMax abs diff (NumPy replica vs expected_out_1.csv): {diff.max():.6f}")
    else:
