...
from qkeras import QDense, QLSTM, quantized_bits, quantized_tanh
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Vanilla KD training for a quantized LSTM Seq2Seq student "
            "using the established frozen GRU teacher."
        ),
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing the production .npy data files.",
    )
    p.add_argument(
        "--save-dir",
        type=str,
        default=None,
        help="Root directory for result subfolders. Defaults to --data-dir.",
    )
    p.add_argument(
        "--seq-len",
        type=int,
    ...
    p.add_argument(
        ...
        type=float,
        default=0.09,
        help="Temporal gate width in nanoseconds.",
    )
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the established frozen GRU teacher weights.",
    )
    p.add_argument(
        "--teacher-units",
        type=int,
        default=128,
        help="Hidden units in each teacher GRU layer.",
    )
    p.add_argument(
        "--teacher-layers",
        type=int,
        default=2,
        help="Number of stacked GRU cells in the teacher.",
    )
    ...
    p.add_argument(
        ...
        default=4.0,
        help=(
            "Retained KD temperature argument. For the regression-MSE KD "
            "objective the temperature factors cancel exactly."
        ),
    )
    p.add_argument(
        "--alpha",
        ...
        default=0.6,
        help=(
            "KD weight. total = alpha * teacher MSE + "
            "(1 - alpha) * target MSE."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=8,
        help="Bits for QLSTM and QDense input kernels.",
    )
    p.add_argument(
        "--bits-bias",
        type=int,
        default=8,
        help="Bits for QLSTM and QDense biases.",
    )
    p.add_argument(
        "--bits-recurrent",
        type=int,
        default=8,
        help="Bits for QLSTM recurrent kernels.",
    )
    p.add_argument(
        "--bits-activation",
        type=int,
        default=8,
        help="Bits for the QLSTM candidate activation.",
    )
    p.add_argument(
        "--bits-state",
        type=int,
        default=8,
        help=(
            "Bits for QLSTM persistent state quantization. QKeras applies "
            "the state quantizer to both h and c before recurrent reuse."
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=32,
        help="Hidden units in the LSTM student encoder and decoder.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1024,
        help="Declared global batch size.",
    )
    p.add_argument(
        "--epochs",
        type=int,
    ...
    p.add_argument(
        ...
        default=1e-4,
        help=(
            "Base learning rate at --ref-batch-size. It is linearly scaled "
            "unless --no-lr-scaling is supplied."
        ),
    )
    p.add_argument(
        "--ref-batch-size",
        type=int,
        default=1024,
        help="Reference batch size for the existing linear LR scaling rule.",
    )
    p.add_argument(
        "--no-lr-scaling",
        action="store_true",
        default=False,
        help="Disable automatic LR, LR-patience, and warmup scaling.",
    )
    p.add_argument(
        "--lr-factor",
        type=float,
    ...
    p.add_argument(
        ...
        type=int,
        default=8,
        help="Validation-plateau patience at the reference batch size.",
    )
    p.add_argument(
        "--lr-min",
        type=float,
    ...
    p.add_argument(
        ...
        type=float,
        default=1e-5,
        help="Minimum validation improvement.",
    )
    p.add_argument(
        "--infer-batch",
        type=int,
        default=8192,
        help="Batch size for teacher caching and final inference.",
    )
    p.add_argument(
        "--mixed-precision",
        action="store_true",
        default=False,
        help=(
            "Accepted for CLI compatibility but disabled by the QKeras "
            "float32 training infrastructure."
        ),
    )
    p.add_argument(
        "--log-interval",
        type=int,
        default=10,
        help="Training progress interval.",
    )
    p.add_argument(
        "--prefetch-batches",
        type=int,
        default=32,
        help="tf.data prefetch depth.",
    )
    p.add_argument(
        "--pipeline-workers",
        type=int,
        default=4,
        help="tf.data map parallelism.",
    )
    p.add_argument(
        "--split-seed",
        type=int,
        default=42,
        help="Training random seed and established split seed.",
    )
    p.add_argument(
        "--warmup-epochs",
        type=int,
        default=5,
        help="Linear learning-rate warmup epochs.",
    )
    p.add_argument(
        "--accumulation-steps",
        type=int,
        default=1,
        help="Gradient-accumulation compatibility argument.",
    )
    p.add_argument(
        "--resume",
        action="store_true",
        default=False,
        help=(
            "Resume from the last completed epoch using the TensorFlow "
            "model+optimizer checkpoint and resume_state.json in the "
            "existing job directory. Missing resume state is fatal."
        ),
    )
    ...
    if args.seq_len <= 0:
        p.error("--seq-len must be positive")
    if args.n_out <= 0:
        p.error("--n-out must be positive")
    if args.student_units <= 0:
        p.error("--student-units must be positive")
    if args.teacher_units <= 0:
        p.error("--teacher-units must be positive")
    if args.teacher_layers <= 0:
        p.error("--teacher-layers must be positive")
    if args.batch_size <= 0:
        p.error("--batch-size must be positive")
    if args.ref_batch_size <= 0:
        p.error("--ref-batch-size must be positive")
    if args.accumulation_steps <= 0:
        p.error("--accumulation-steps must be positive")
    if args.batch_size % args.accumulation_steps != 0:
        p.error(
            "--batch-size must be exactly divisible by --accumulation-steps"
        )
    if args.epochs <= 0:
    ...
    if args.lr_min <= 0.0:
        p.error("--lr-min must be positive")
    if not 0.0 < args.lr_factor < 1.0:
        p.error("--lr-factor must be between 0 and 1")
    if args.min_delta < 0.0:
        p.error("--min-delta cannot be negative")
...
def train_step_per_replica(
    ...
    temperature_f = tf.cast(temperature, tf.float32)
    ...
    with tf.GradientTape() as tape:
        ...
        hard_loss = tf.reduce_mean(
            tf.square(student_output - tgt_b)
        )
        ...
        soft_loss = (
            ...
            * tf.reduce_mean(
                tf.square(
                    tpred_b / temperature_f
                    - student_output / temperature_f
                )
            )
        )
    ...
    grads = [
        tf.zeros_like(variable) if grad is None else grad
    ...
    all_finite = tf.reduce_all(
        tf.stack(finite_flags)
    )
    nonfinite_flag = tf.logical_not(all_finite)
    ...
    sanitized_grads, _ = tf.clip_by_global_norm(
        sanitized_grads,
        clip_norm=1.0,
    )
    ...
    return (
        ...
        hard_loss,
        soft_loss,
        tf.cast(nonfinite_flag, tf.float32),
    )
...
def training_loop(
    ...
    nan_warn_threshold = max(
        1,
        int(train_steps * 0.10),
    )
    ...
    checkpoint_manager = tf.train.CheckpointManager(
        checkpoint,
        directory=resume_ckpt_dir,
        max_to_keep=1,
        checkpoint_name="last_epoch",
    )

    def write_resume_state(epoch_completed):
        state = {
            "epoch": int(epoch_completed),
            "best_val": float(best_val),
            "patience_ct": int(patience_ct),
            "lr": float(lr_scheduler.current_lr),
            "lr_best": float(lr_scheduler.best),
            "lr_wait": int(lr_scheduler.wait),
            "training_complete": bool(
                epoch_completed >= args.epochs
                or patience_ct >= args.patience
            ),
            "latest_checkpoint": (
    ...
    if args.resume:
        ...
        if missing:
            raise RuntimeError(
                "--resume was requested but required "
                "resume files are missing:\n"
                + "\n".join(
                    f"  {path}"
                    for path in missing
                )
        ...
        if missing_keys:
            raise RuntimeError(
                "resume_state.json is missing required keys: "
                + ", ".join(missing_keys)
            )

        pf(
            "[RESUME] Restoring last completed epoch "
            f"checkpoint: "
        ...
        best_val = float(
            resume_state["best_val"]
        )
        patience_ct = int(
            resume_state["patience_ct"]
        )
        saved_lr = float(
            resume_state["lr"]
        )

        lr_scheduler.lr_var.assign(
            saved_lr
        )
        lr_scheduler.best = float(
            resume_state.get(
                "lr_best",
                best_val,
            )
        )
        lr_scheduler.wait = int(
            resume_state.get(
                "lr_wait",
                0,
            )
        ...
        for key in history:
            if key not in saved_history:
                raise RuntimeError(
                    "resume_state.json history "
                    f"is missing key: {key}"
                )

            history[key] = [
                float(value)
                for value in saved_history[key]
            ]

            if len(history[key]) != start_epoch:
                raise RuntimeError(
                    "resume history length mismatch "
                    f"for {key}: "
                    f"len={len(history[key])}, "
                    f"epoch={start_epoch}"
                )
        ...
        pf(
            ...
            f"patience={patience_ct}/{args.patience}  "
            f"lr={saved_lr:.2e}  "
            f"lr_wait={lr_scheduler.wait}"
        )
        sys.stdout.flush()
    ...
    pf(
        ...
        "(forward-only, no weight update)..."
    )

    try:
        sample_batch_x, sample_batch_y = next(
            iter(dist_train_dataset)
        )
        del sample_batch_y
        ...
        _ = student_model(
            [
                sample_batch_x["enc_input"],
                sample_batch_x["dec_input"],
            ],
            training=False,
        )
        ...
        status_text = (
            ...
            else "SLOW - check GPU visibility"
        )

        pf(
            "  Single forward batch: "
            f"{elapsed_preflight:.3f}s  "
            f"[{status_text}]"
        )
    ...
    pf("=" * 60)
    pf("VANILLA KD LSTM STUDENT TRAINING")
    ...
    pf(
        f"  Effective LR        : "
        f"{args.effective_lr:.2e}"
    )
    pf(
        f"  LR patience         : "
        f"{args.effective_lr_patience}"
    )
    pf(
        f"  Warmup epochs       : "
    ...
    pf(
        f"  Student QLSTM       : "
        f"{args.student_units} units"
    )
    pf(
        "  bits                : "
        f"k={args.bits_kernel} "
        f"r={args.bits_recurrent} "
        f"b={args.bits_bias} "
        f"a={args.bits_activation} "
        f"s={args.bits_state}"
    ...
    if start_epoch == 0:
        with open(csv_path, "w") as csv_f:
            csv_f.write(
                ...
                "val_total,val_hard,val_soft,"
                "val_mae,lr\n"
            )

    elif not os.path.exists(csv_path):
        raise RuntimeError(
            "training_history.csv is missing "
            f"during resume: {csv_path}"
        )
    ...
    if already_complete:
        pf(
            "[RESUME] Training is already complete: "
            f"epoch={start_epoch}, "
            f"patience={patience_ct}. "
            "Skipping directly to best-weight restore."
        )
    ...
    for epoch in range(
        ...
        if (
            args.effective_warmup_epochs > 0
            and epoch
            < args.effective_warmup_epochs
        ):
            epoch_lr = (
                float(args.effective_lr)
                * float(epoch + 1)
                / float(
                    args.effective_warmup_epochs
                )
        ...
        for step, (bx, by) in enumerate(
            ...
            hard_value = float(hard_l)
            soft_value = float(soft_l)
            ...
            if (
                not np.isfinite(hard_value)
                or not np.isfinite(soft_value)
            ):
                raise FloatingPointError(
                    "non-finite component loss at "
                    f"epoch {epoch + 1}, "
                    f"step {step + 1}: "
                    f"hard={hard_value}, "
                    f"soft={soft_value}"
        ...
        if acc_steps != train_steps:
            raise RuntimeError(
                "training dataset produced "
                f"{acc_steps} steps; "
                f"expected {train_steps}"
            )
        ...
        if nan_count > 0:
            pf(
                f"WARNING: {nan_count}/"
                f"{acc_steps} training batches "
                "reported non-finite gradients."
            )

        if nan_count > nan_warn_threshold:
            raise FloatingPointError(
                "non-finite gradients exceeded "
                "threshold: "
                f"{nan_count}/{acc_steps} batches"
            )
        ...
        for bx, by in dist_val_dataset:
            ...
            val_acc_total += values[0]
            val_acc_hard += values[1]
            val_acc_soft += values[2]
            val_acc_mae += values[3]
            val_done += 1

        if val_done != val_steps:
            raise RuntimeError(
                "validation dataset produced "
                f"{val_done} steps; "
                f"expected {val_steps}"
            )

        val_loss = (
            val_acc_total / val_done
        )
        val_hard = (
            val_acc_hard / val_done
        )
        val_soft = (
            val_acc_soft / val_done
        )
        val_mae = (
            val_acc_mae / val_done
        )
        ...
        history["val_hard"].append(
            val_hard
        )
        history["val_soft"].append(
            val_soft
        )
        ...
        pf(
            ...
            f"val={val_loss:.6f}  "
            f"hard={train_hard:.6f}  "
            f"soft={train_soft:.6f}  "
...
def save_loss_curves(
    ...
    axes[2].plot(
        history["soft"],
        color="tab:green",
        label="train teacher MSE",
    )
    axes[2].plot(
        history["val_soft"],
        color="tab:red",
        linestyle="--",
        label="val teacher MSE",
    )
    axes[2].set_title(
        "Teacher-output MSE"
    )
    ...
    axes[3].text(
        ...
        0.55,
        (
            f"Vanilla KD LSTM  SEQLEN={args.seq_len}\n"
            f"alpha={args.alpha}  regression MSE KD\n"
            f"Teacher GRU hidden={args.teacher_units} "
            f"x {args.teacher_layers} layers\n"
            f"Student QLSTM hidden={args.student_units}\n"
            f"bits: k={args.bits_kernel} "
            f"r={args.bits_recurrent} "
            f"b={args.bits_bias} "
            f"a={args.bits_activation} "
            f"s={args.bits_state}\n"
            ...
            f"Base LR={args.lr:.2e}  "
            f"Effective LR={args.effective_lr:.2e}\n"
            f"Best val loss={best_val_loss:.6f}\n"
            f"Epochs completed={len(history['total'])}"
        ),
        fontsize=9,
    ...
    pf(
        f"Loss curves saved: {curves_path}"
    )
...
def main():
    ...
    pf(
        f"  Student QLSTM-{args.student_units}  "
        f"{args.bits_kernel}-bit kernel"
    )
